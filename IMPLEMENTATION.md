...
Load all resources in a directory and index by `metadata.id`:

```
1. Scan directory for .yml/.yaml/.json files
2. Parse each file as a resource
3. Validate against schema for resource kind
4. Index by metadata.id
5. Check for duplicate IDs (error if found)
```
...
```
function resolveBody(body, fragmentIndex):
  if body is string:
    return body
  ...
  if body is array:
    ...
    for item in body:
      if item is string:
        ...
        fragment = fragmentIndex[item.fragment]
        if not fragment:
          error("Fragment not found: " + item.fragment)
        
        validateInputs(fragment.spec.inputs, item.inputs)
        rendered = renderTemplate(fragment.spec.body, item.inputs)
...
Error: Fragment 'read-file' referenced in Prompt 'implement' not found
...
Error in prompts.yml:
  Line 8: Fragment 'read-file' not found in directory
```
...
2. Resolve fragments by `metadata.id` from directory
